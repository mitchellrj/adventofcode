...
def main(input_lines):
    ...
    # Now walk the tree to find the sizes
    _, total_size_of_directories_under_or_equal_to_limit = size_calculation_walker_reducer(root, 0)
    return total_size_of_directories_under_or_equal_to_limit


def size_calculation_walker_reducer(current_node, running_total):
    ...
    if current_node.size != None:
        return current_node.size, running_total

    total = 0
    for node in current_node.children.values():
        child_total, running_total = size_calculation_walker_reducer(node, running_total)
        total += child_total
